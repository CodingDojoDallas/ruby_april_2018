class BankAccount
	attr_accessor :acct_num, :chk_bal, :save_bal, :money
	@@total_accts = 0
	...
	def deposit money, acct
		if acct == "checking"
			@chk_bal += money
			p "You have deposited $#{money} into your #{acct} account"
		elsif acct == "savings"
			@save_bal += money
			p "You have deposited $#{money} into your #{acct} account"
		else
	...
	def withdraw money, acct
		if acct == "checking" and @chk_bal > money
			@chk_bal -= money
			p "You have withdrawn $#{money} from your #{acct} account"
		elsif acct == "savings" and @save_bal > money
			@save_bal -= money
			p "You have withdrawn $#{money} from your #{acct} account"
		else
			p "Unable to process your request, please contact your account administrator."
		end
		self
	end
